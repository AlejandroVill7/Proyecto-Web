<x-app-layout>
    ...
    <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-80 rounded-xl bg-white shadow-xl ring-1 ring-black ring-opacity-5 z-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-0 overflow-hidden" style="display: none;">
                    <div class="p-4 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Arrastra los widgets para reordenarlos. Usa este menú para ocultar o cambiar tipos.</p>
                    </div>
                    
                    <form id="preferencesForm" class="max-h-[400px] overflow-y-auto p-4 space-y-4">
                        {{-- Section: Stats Cards --}}
                        <div>
                            <h4 class="text-xs font-bold uppercase text-gray-400 mb-2 tracking-wider">Tarjetas de Resumen</h4>
                            <div class="space-y-2">
                                @foreach($widgets as $widget)
                                    @if(str_contains($widget['key'], 'stats'))
                                        <label class="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer border border-transparent hover:border-gray-200 dark:hover:border-gray-600 transition">
                        ...
                        <div>
                            <h4 class="text-xs font-bold uppercase text-gray-400 mb-2 tracking-wider">Gráficos y Listas</h4>
                            <div class="space-y-3">
                                @foreach($widgets as $widget)
                                    @if(!str_contains($widget['key'], 'stats'))
                                        <div class="p-3 rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
                                            ...
                                            @if(str_contains($widget['key'], 'chart'))
                                                <div class="ml-6 space-y-2">
                                                    {{-- Chart Type Selector --}}
                                                    <div>
                                                        <label class="text-xs text-gray-500 dark:text-gray-400 block mb-1">Tipo de Gráfico:</label>
                                                        <select class="w-full text-xs rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 widget-type" data-key="{{ $widget['key'] }}">
                                                            <option value="bar" {{ ($widget['settings']['type'] ?? '') == 'bar' ? 'selected' : '' }}>Barras (Vertical)</option>
                                                            <option value="horizontalBar" {{ ($widget['settings']['type'] ?? '') == 'horizontalBar' ? 'selected' : '' }}>Barras (Horizontal)</option>
                                                    ...
                                                    @if($widget['key'] == 'chart_evaluacion')
                                                        <div>
                                                            <label class="text-xs text-gray-500 dark:text-gray-400 block mb-1">Filtrar por Evento:</label>
                                                            <select class="w-full text-xs rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 widget-event" data-key="{{ $widget['key'] }}">
                                                                <option value="">Todos los Eventos</option>
                                                                @foreach($eventos_stats as $evt)
                                                                    <option value="{{ $evt['id'] }}" {{ ($widget['settings']['event_id'] ?? '') == $evt['id'] ? 'selected' : '' }}>
                    ...
                    <div class="p-4 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-100 dark:border-gray-700 text-right">
                        <button type="button" id="saveConfigBtn" class="w-full bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition font-medium shadow-sm">
                            Aplicar Cambios
                        </button>
                    </div>
                </div>
        ...
        @foreach($widgets as $widget)
            @if($widget['is_visible'])
                <div class="widget-item relative group {{ str_contains($widget['key'], 'chart') || str_contains($widget['key'], 'list') ? 'col-span-1 md:col-span-2' : 'col-span-1' }}" data-key="{{ $widget['key'] }}">
                    ...
                    {{-- Drag Handle --}}
                    <div class="drag-handle absolute top-3 right-3 p-1.5 rounded-md bg-white/80 dark:bg-gray-800/80 text-gray-400 hover:text-indigo-600 cursor-move opacity-0 group-hover:opacity-100 transition z-10 shadow-sm backdrop-blur-sm border border-gray-200 dark:border-gray-600">
                    ...
                    @switch($widget['key'])
                            ...
                            <div class="h-full rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    @if(!empty($widget['settings']['event_id']))
                                        <span class="text-xs bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 px-2 py-1 rounded-full">
                                            {{ $eventos_stats->where('id', $widget['settings']['event_id'])->first()['nombre'] ?? 'Evento' }}
                                        </span>
                                    @endif
                                </div>
                                <div class="relative h-72 w-full">
                                    <canvas id="chartEvaluacion" 
                                            data-type="{{ $widget['settings']['type'] ?? 'bar' }}"
                        ...
                        @case('chart_categorias')
                            <div class="h-full rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                ...
                                <div class="relative h-72 w-full flex justify-center">
                                    <canvas id="chartCategorias" data-type="{{ $widget['settings']['type'] ?? 'bar' }}"></canvas>
                                </div>
                            </div>
                            @break
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const palette = [colors.primary, colors.secondary, colors.success, colors.warning, colors.info, colors.purple];
            ...
            const getCommonOptions = (type) => {
                ...
                return {
                    ...
                    plugins: {
                        legend: {
                            display: type !== 'bar' && type !== 'horizontalBar', // Hide legend for standard bars
                            position: 'bottom',
                            labels: {
                                color: theme.text,
                                padding: 20,
                    ...
                    scales: (type === 'doughnut' || type === 'pie') ? {} : {
                        ...
                        y: {
                            beginAtZero: true,
                            grid: { color: theme.grid, borderDash: [4, 4], drawBorder: false },
                            ticks: { color: theme.text, font: { size: 11 }, padding: 10 }
                        }
                    },
                    layout: { padding: 10 }
                };
            ...
            if (ctxEvaluacion) {
                const type = ctxEvaluacion.dataset.type === 'horizontalBar' ? 'bar' : (ctxEvaluacion.dataset.type || 'bar');
                ...
                if (type === 'doughnut' || type === 'pie') {
                    ...
                    chartData.datasets[0].backgroundColor = [colors.primary, isDark() ? colors.darkGray : colors.lightGray];
            ...
            if (ctxCarreras) {
                const type = ctxCarreras.dataset.type === 'horizontalBar' ? 'bar' : (ctxCarreras.dataset.type || 'doughnut');
            ...
            if (ctxPendientes) {
                const type = ctxPendientes.dataset.type === 'horizontalBar' ? 'bar' : (ctxPendientes.dataset.type || 'line');
                ...
                new Chart(ctxPendientes, {
                    ...
                    options: {
                        ...getCommonOptions(type),
                        plugins: {
                            legend: { display: false } // Usually no legend needed for single series
                        },
                        scales: {
                            x: {
                                grid: { display: false },
            ...
            if (ctxCategorias) {
                const type = ctxCategorias.dataset.type === 'horizontalBar' ? 'bar' : (ctxCategorias.dataset.type || 'bar');
                ...
                new Chart(ctxCategorias, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            data: values,
                            backgroundColor: palette,
                            borderColor: (type === 'doughnut' || type === 'pie') ? 'transparent' : (isDark() ? '#1f2937' : '#ffffff'),
                            borderWidth: 2,
                            borderRadius: 4,
                            barPercentage: 0.6,
                            hoverOffset: 4
